...
if __name__=="__main__":
    ...
    output_path = STORAGE_ROOT / (benchmark_name+"_16") / f"{benchmark_name}_idx-{conf['split_idx']}_method-{conf['bounding_method']}_mask-{conf['split_mask']}_mode-{args.mode}_seed-{args.seed}"
    ...
    if args.mode == "conv":
        ...
        with open(output_f, "w") as f:
            ...
            for instance in instances:
                ...
                conf['n_splits'] = 8
                new_model, baseline = splitter.split(1, args.mode , conf)
                new_model.save(TOOL_ROOT / f"temp_split.onnx")
                baseline.save(TOOL_ROOT / f"temp_baseline.onnx")
                
                splitter = ReluSplitter(TOOL_ROOT / f"temp_split.onnx", vnnlib_path, conf=conf)
                new_model, _ = splitter.split(0, args.mode , conf)
                splitter = ReluSplitter(TOOL_ROOT / f"temp_baseline.onnx", vnnlib_path, conf=conf)
                _, baseline = splitter.split(0, args.mode , conf)
