...
class ReluSplitter():
    ...
    def split_iterative(self, split_idxs=None, max_splits=None, split_strategy="single"):
        if split_idxs is None:
            split_idxs = range(len(self.get_splitable_nodes()))
        splitable_nodes = self.get_splitable_nodes()
        ...
        for split_idx in split_idxs:
            ...
            # the input and output of the Gemm node
            input_lb, input_ub = model.get_bound_of(self.bounded_input, gemm_node.input[0])          # the input bound of the Gemm node, from which the split location is sampled
            output_lb, output_ub = model.get_bound_of(self.bounded_input, relu_node.output[0])       # the output bound for determining the stability of the neurons
            ...
            model = model.generate_updated_model( nodes_to_replace=[gemm_node, relu_node],
                                                        additional_nodes=new_nodes,
                                                        additional_initializers=new_initializers,
                                                        graph_name=f"{self.onnx_path.stem}_split_{split_idx}",
                                                        producer_name="ReluSplitter")
            model.save(Path(f"iter_{split_idx}.onnx"))
            self.logger.info("Model created")
        
        self.logger.info("Checking model Correctness")
        input_shape = list(self.warpped_model.input_shapes.values())[0]
        equiv, diff = check_model_closeness(self.warpped_model, model, input_shape, atol=1e-4, rtol=1e-4)
        if not equiv:
            self.logger.error("Model closeness check failed, proceed with caution")
            raise ValueError("Model closeness check failed")
        else:
